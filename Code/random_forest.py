...
IMAGE_SIZE = 256
image_dir = "D:\\GLOF\\Code"  # Change this to your dataset path
mask_dir = "D:\\GLOF\\random_forest"  # Directory to save masks
...
def load_dataset(image_dir, mask_dir):
    ...
    for file in os.listdir(image_dir):
        ...
        if img is None:
            print(f"❌ Warning: Could not read {img_path}, skipping.")
            continue
        img = cv2.resize(img, (IMAGE_SIZE, IMAGE_SIZE)).reshape(-1)
        ...
        if mask is None:
            ...
            continue
        mask = cv2.resize(mask, (IMAGE_SIZE, IMAGE_SIZE)).reshape(-1)
...
if X.shape[0] != Y.shape[0]:
    raise ValueError("Mismatch: Number of images and masks do not match!")

# Ensure each mask has only one label per image
if Y.shape[1] != 1:
    Y = np.mean(Y, axis=1, keepdims=True)  # Convert mask to a single label per image

# Convert to integers for classification
Y = (Y > 0.5).astype(int)  # Ensure binary classification labels (0 or 1)
